...
DELAYL ADD R0,R0,#-1 ; a delay so that the game isnt so fast
 ...
 ;key press
 LDI R0,KB_STE
 BRnp JUMP1 ;skip key stuff and set vel to 0 if status is zero
 AND R0,R0,#0
 ST R0,PLAYER_VEL
 BRnzp ENDKEY
JUMP1
...
BADPOS
 ...
 LDR R0,R0,#0
 ADD R0,R0,R3
 BRz JUMP4 ;skip next 4 lines if color is the background
 LD R0,BALL_VEL_X
 NOT R0,R0
...
JUMP4
 ...
 LDR R0,R0,#0
 ADD R3,R3,R0
 BRz JUMP5 ;skip next 14 lines if color is the background

 LD R3,PLAYER_COL_I
 ADD R3,R3,R0
 BRnp JUMP6 ;skip next 7 lines if color is not player color
 JSR PADDLEDIFF
 LD R1,BALL_VEL_X
 ADD R0,R0,R1
